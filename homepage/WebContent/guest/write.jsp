...
<body>
	...
	<div align="center">
		...
		<form class=box name="writeForm" action="writeOk.jsp" method="post"
			onsubmit="return writeCheck(this)">
			<div class=div_top>
				<font>이름</font> <font><input type="text" name="name" /></font> <font>비밀번호</font>
			...
			<div class=div_top style="text-align: right;">
				<font><input type="submit" value="확인"></font> <font><input
					type="reset" value="취소"></font>
			</div>
		</form>
		<%} %>
		<%if(guestList != null){ %>
		...
		<div class="box" style="border-width: 1px;">
			<div class="div_top" style="text-align: right; border-width: 1px">
				<%=list.getName() %>
				<%=sdf.format(list.getWriteDate()) %>&nbsp;&nbsp;&nbsp;&nbsp; <a
					href="update.jsp?num=<%=list.getNum()%>">수정</a> <a
					href="delete.jsp?num=<%=list.getNum() %>">삭제</a>
			</div>
	...
	<a href="write.jsp?pageNumber=<%=i%>">[<%=i %>]
	</a>
	<%} %>
